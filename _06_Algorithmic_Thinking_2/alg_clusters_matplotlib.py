...
def plot_clusters(data_table, cluster_list, draw_centers=False):
    ...
    if not draw_centers:
        for cluster_idx in range(len(cluster_list)):
            ...
            for fips_code in cluster.fips_codes():
                ...
                plt.scatter(x=[line[1]], y=[line[2]], s=circle_area(line[3]), lw=1, facecolors=cluster_color, edgecolors=cluster_color)
    ...
    # add cluster centers and lines from center to counties
        ...
        for cluster_idx in range(len(cluster_list)):
            ...
            plt.scatter(x=[cluster_center[0]], y=[cluster_center[1]], s=circle_area(cluster_pop), lw=2, facecolors="none", edgecolors="black", zorder=3)
