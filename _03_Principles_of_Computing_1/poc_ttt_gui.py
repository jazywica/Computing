...
class TicTacGUI:
    ...
    def setup_frame(self):
        ...
        self._frame = simplegui.create_frame("Tic-Tac-Toe", GUI_WIDTH, GUI_HEIGHT)
    ...
    def drawx(self, canvas, pos):
        ...
        canvas.draw_line((pos[0] - halfsize, pos[1] - halfsize), (pos[0] + halfsize, pos[1] + halfsize), BAR_WIDTH, 'Black')
        canvas.draw_line((pos[0] + halfsize, pos[1] - halfsize), (pos[0] - halfsize, pos[1] + halfsize), BAR_WIDTH, 'Black')
    ...
    def aimove(self):
        ...
        if self._inprogress and (self._turn == self._aiplayer):
            row, col = self._aifunction(self._board, self._aiplayer, self._ntrials)
            if self._board.square(row, col) == provided.EMPTY:
    ...
    def get_coords_from_grid(self, row, col):
        """ Given a grid position in the form (row, col), returns the coordinates on the canvas of the center of the grid. """
