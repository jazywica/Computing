# -*- encoding: utf-8 -*-
""" MEMORY - First project of Week 1 made with objects """
# use following link to test the program in 'codeskulptor': http://www.codeskulptor.org/#user45_dX6VwBU3iO_3.py
...
def new_game():
    ...
    # my_tiles = []  # this is the same as the line above - it creates a list with Tile type objects
    # for i in range(2 * DISTINCT_TILES)):
    #     tile = Tile(numbers[i], False, [i * TILE_WIDTH, TILE_HEIGHT])
...
def mouseclick(pos):
    ...
    for tile in my_tiles:
        if tile.is_selected(pos):
            clicked_tile = tile
    if clicked_tile.is_exposed():  # this will stop the whole procedure if the card is already exposed
        return
...
# draw handler
def draw(canvas):
    for tile in my_tiles:  # in the object version we simply call the 'draw_tile' method
