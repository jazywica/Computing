""" GUI component of Tantrix Solitaire implementation: http://www.jaapsch.net/puzzles/tantrix.htm """
#http://www.codeskulptor.org/#poc_tantrix_gui.py
...
class TantrixGUI:
    ...
    def init_grid(self):
        ...
        self.corners = [[EDGE_LENGTH, 0.5 * HEX_HEIGHT], [EDGE_LENGTH, (self._tiling_size + 0.5) * HEX_HEIGHT],
                        [EDGE_LENGTH + (3 * self._tiling_size * EDGE_LENGTH / 2), 0.5 * (self._tiling_size + 1) * HEX_HEIGHT]]
    ...
    def draw_tile(self, canvas, center, code):
        """ Draw a tile based on its center and code. Credit to Scott R. for his implementation of circular arcs """
        hexagon = make_hexagon(center)
        canvas.draw_polygon(hexagon[:], 2, "White", "Black")

        mid_pts = [[0.5 * (hexagon[idx][dim] + hexagon[idx + 1][dim]) for dim in range(2)] for idx in range(len(hexagon) - 1)]
