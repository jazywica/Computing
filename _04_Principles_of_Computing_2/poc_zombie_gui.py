""" Zombie Apocalypse mini-project. Click "Mouse click" button to toggle items added by mouse clicks. Zombies have four way movement, humans have eight way movement """
...
NAME_MAP = {OBSTACLE: "obstacle", HUMAN: "human", ZOMBIE: "zombie"}
...
class ApocalypseGUI:
    ...
    def flee(self):
        """ Event handler for button that causes humans to flee zombies by one cell. Diagonal movement allowed !!!!! """
        zombie_distance = self._simulation.compute_distance_field(ZOMBIE)
        self._simulation.move_humans(zombie_distance)

    def stalk(self):
        """ Event handler for button that causes zombies to stack humans by one cell. Diagonal movement not allowed !!!!! """
