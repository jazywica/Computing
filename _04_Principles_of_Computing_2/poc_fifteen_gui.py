...
# http://www.codeskulptor.org/#poc_fifteen_gui.py
...
class FifteenGUI:
    ...
    def __init__(self, puzzle):
        ...
        self._frame = simplegui.create_frame("The Fifteen puzzle", self._puzzle_width * TILE_SIZE, self._puzzle_height * TILE_SIZE)
        self._solution = ""
        self._current_moves = ""
        ...
        for row in range(self._puzzle_height):
            for col in range(self._puzzle_width):
                ...
                if tile_num == 0:
                    ...
                    background = "Blue"
                tile = [[col * TILE_SIZE, row * TILE_SIZE], [(col + 1) * TILE_SIZE, row * TILE_SIZE], [(col + 1) * TILE_SIZE, (row + 1) * TILE_SIZE], [col * TILE_SIZE, (row + 1) * TILE_SIZE]]
                canvas.draw_polygon(tile, 1, "White", background)
                canvas.draw_text(str(tile_num), [(col + .2) * TILE_SIZE, (row + 0.8) * TILE_SIZE], 2 * TILE_SIZE // 3, "White")
